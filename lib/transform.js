...
if (typeof process === 'undefined' || process.browser) {
  SourceMapConsumer.initialize({
    "lib/mappings.wasm": "https://unpkg.com/source-map@0.8.0-beta.0/lib/mappings.wasm"
  });
}
  ...
  if (sourceMap && unit.sourceMap) {
    ...
    if (unit.sourceMap.mappings !== '') {
      p = Promise.all([
        ...
        try {
          // Bypass source-map's soureRoot bug
          if (typeof newConsumer.sourceRoot !== 'string') newConsumer.sourceRoot = "";
          const generator = SourceMapGenerator.fromSourceMap(newConsumer);
          generator.applySourceMap(oldConsumer);
          merged.sourceMap = generator.toJSON();
        } catch (err) {
