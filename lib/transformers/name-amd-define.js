...
module.exports = function nameAmdDefine(unit) {
  ...
  const result = ensureNamedDefine(unit, parsed);

  const amd = usesAmdOrRequireJs(parsed, globalIndentifiers(parsed));
  if (amd) {
    if (amd['require']) {
  ...
  dynamicDepFinder(parsed).forEach(d => {
...
function ensureNamedDefine(unit, ast) {
  ...
  let definesIdentifiers = globals.define;
  ...
  if (defineCalls.length > 1) {
    ...
    defineCalls.forEach(function (match) {
      ...
      if (args[0] && (args[0].type === 'Literal' || args[0].type === 'StringLiteral')) {
    ...
    if (isUMD(ast)) {
      // A special UMD module which calls
      // define() indirectly through passed param.
      let deps = [];
      const cjs = usesCommonJs(ast, globals);
      if (cjs && cjs['require']) {
        deps = cjs['require'];
      }
      return {
