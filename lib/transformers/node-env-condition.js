...
// The runtime value is handled by ./process.js
...
module.exports = function nodeEnvCondition(unit, _nodeEnv /* for test */) {
  if (! /process\.env(?:\.|\[\s*['"])NODE_ENV/.test(unit.contents)) return;
  ...
  if (result.code === contents) {
    // no change, reuse the parsed ast
    return {parsed};
  }
...
function nodeEnvCheck(exp, _nodeEnv) {
  ...
  let targetValue;
  let possibleRef;
  if (left.type === 'Literal' || left.type === 'StringLiteral') {
    targetValue = left.value;
    possibleRef = right;
  } else if (right.type === 'Literal' || right.type === 'StringLiteral') {
