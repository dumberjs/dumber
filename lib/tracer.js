...
Tracer.prototype.packageReaderFor = function (packageConfig) {
  ...
  return this.packageLocator(packageConfig).then(function (locator) {
...
Tracer.prototype.resolve = function () {
  ...
  for (let i = 0; i < len; i += 1) {
    ...
    p = p.then(function () {
      return self.packageReaderFor({name: packageName});
