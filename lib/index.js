...
const {generateHash, stripJsExtension, contentOrFile} = require('./shared');
...
const printVersion = (function() {
  ...
  return function() {
    if (did) return;
    did = true;
    info(`Starting dumber bundler v${version} https://dumber.js.org`);
  };
})();
...
// 3. bundle: write to bundles (objects, not final file contents)
...
module.exports = class Bundler {
  constructor(opts, mock) {
    ...
    this._resolve = (mock && mock.resolve) || resolvePackage;
    ...
    if (!opts.skipModuleLoader) {
      this._prepends.push(
        ...
        path.join(path.relative(process.cwd(), this._resolve('dumber-module-loader')), 'dist', 'index.debug.js')
      );
    }
  ...
  packageReaderFor(packageConfig) {
    ...
    return this._fileReader(packageConfig).then(fileReader => {
      const reader = new PackageReader(fileReader, this._needsSourceMap);
  ...
  capture(unit) {
    if (!this._needsSourceMap && unit.sourceMap) {
      this._needsSourceMap = true;
    }
  ...
  _supportInjectCssIfNeeded() {
    ...
    return this.capture({
      ...
      contents: "define(['dumber/lib/inject-css'],function(m){return m;});",
      moduleId: 'ext:css',
  ...
  _onAcquire(id, opts) {
    ...
    if (parsedId.bareId.match(/^(?:https?:)?\//)) {
      // ignore https:// and /root/path.
      // they are for runtime loading.
      return Promise.resolve();
    }
  ...
  _unitsForBundle(bundle) {
    ...
    // in AMD module environment. There is special code in lib/transformers/hack-moment.js
